# Unrolled Inline Assembler Micro Kernel

We now manually unroll the assembler micro kernel.

The basic transformation is straightforward. The loop
...
The `[code for ...]` blocks are essentially just copied and pasted.

This is the same loop-unrolling idea that we already applied to the intrinsic implementation. The difference is that we now have complete control over the resulting instruction sequence.

## Select the `demo-sse-asm-unrolled` Branch
...
Now check out the `demo-sse-asm-unrolled` branch:

```console
$ git checkout -B demo-sse-asm-unrolled remotes/origin/demo-sse-asm-unrolled
Switched to a new branch 'demo-sse-asm-unrolled'
Branch demo-sse-asm-unrolled set up to track remote branch demo-sse-asm-unrolled from origin.
```
...
[`src/level3/dgemm_nn.c`](https://github.com/michael-lehn/ulmBLAS/blob/demo-sse-asm-unrolled/src/level3/dgemm_nn.c)

on the `demo-sse-asm-unrolled` branch.

The unrolling is implemented by splitting `kc` into blocks of four and a remainder:
...
The assembler therefore contains two loops:
...
The main loop performs four consecutive rank-one updates before testing the loop condition again.
...
If `kc` is not divisible by four, the remaining `kc % 4` updates are handled by a second loop:
...
After both loops, the delayed accumulator updates are completed and the resulting $4\times4$ block is written back to `AB`.

The complete assembler block is rather long because the body of the update loop has literally been replicated four times. For that reason, the complete source is best viewed directly in `dgemm_nn.c`.
...
==== = = ==== ==== ==== ===== ==== ==== ===== ==== ===== ===== ==== =====
   0 N N  100  100  100   1.0 1000 1000   1.0 1000  0.00 1805.1 1.00 -----
   0 N N  100  100  100   1.0 1000 1000   1.0 1000  0.00 5586.6 3.09 PASS
   1 N N  200  200  200   1.0 1000 1000   1.0 1000  0.01 1962.5 1.00 -----
   1 N N  200  200  200   1.0 1000 1000   1.0 1000  0.00 6968.6 3.55 PASS
   2 N N  300  300  300   1.0 1000 1000   1.0 1000  0.03 2018.2 1.00 -----
   2 N N  300  300  300   1.0 1000 1000   1.0 1000  0.01 7298.3 3.62 PASS
   3 N N  400  400  400   1.0 1000 1000   1.0 1000  0.07 1926.9 1.00 -----
   3 N N  400  400  400   1.0 1000 1000   1.0 1000  0.02 7125.8 3.70 PASS
   4 N N  500  500  500   1.0 1000 1000   1.0 1000  0.12 2017.6 1.00 -----
   4 N N  500  500  500   1.0 1000 1000   1.0 1000  0.03 7176.9 3.56 PASS
   5 N N  600  600  600   1.0 1000 1000   1.0 1000  0.34 1260.3 1.00 -----
   5 N N  600  600  600   1.0 1000 1000   1.0 1000  0.06 7559.5 6.00 PASS
   6 N N  700  700  700   1.0 1000 1000   1.0 1000  0.66 1043.1 1.00 -----
   6 N N  700  700  700   1.0 1000 1000   1.0 1000  0.09 7642.7 7.33 PASS
   7 N N  800  800  800   1.0 1000 1000   1.0 1000  1.01 1009.0 1.00 -----
   7 N N  800  800  800   1.0 1000 1000   1.0 1000  0.14 7469.5 7.40 PASS
   8 N N  900  900  900   1.0 1000 1000   1.0 1000  1.45 1007.9 1.00 -----
   8 N N  900  900  900   1.0 1000 1000   1.0 1000  0.19 7614.2 7.55 PASS
   9 N N 1000 1000 1000   1.0 1000 1000   1.0 1000  1.83 1094.8 1.00 -----
   9 N N 1000 1000 1000   1.0 1000 1000   1.0 1000  0.26 7630.3 6.97 PASS
...
Filter out the results for the `demo-sse-asm-unrolled` branch:

```console
$ grep PASS report > demo-sse-asm-unrolled
```

The gnuplot script `bench10.gps` is:

```gnuplot
set terminal svg size 1140,480 background rgb 'white'
set output "bench10.svg"
...
![Benchmark results](bench10.svg)

The unrolled assembler implementation reaches about **7.6 GFLOPS** on the original benchmark machine. At $N=1000$, the measured performance is **7630.3 MFLOPS**.

Compared with the previous assembler implementation, which reached about 7.0 GFLOPS, manually unrolling the loop gives another measurable improvement.

---

[← Previous](../page08/README.md) | [Main Page](../README.md) | [Next →](../page10/README.md)
